...
#warning "gmp is not used for memory leak, please use gmc instead."
  ...
  srs_trace(
      "cwd=%s, work_dir=%s, build: %s, configure: %s, uname: %s, osx: %d, pkg: %s, region: %s, "
      "source: %s",
      _srs_config->cwd().c_str(), cwd.c_str(), SRS_BUILD_DATE, SRS_USER_CONFIGURE, SRS_UNAME,
      SRS_OSX_BOOL, SRS_PACKAGER, srs_getenv("SRS_REGION").c_str(),
      srs_getenv("SRS_SOURCE").c_str());
  ...
  if (true) {
    ...
    ss << "glic mem-check env MALLOC_CHECK_ " << mcov << "=>" << mcnv << ", LIBC_FATAL_STDERR_ "
       << lfsov << "=>" << lfsnv << ".";
#endif
    ...
    if (hcov.empty()) {
      ...
      srs_warn("gmc HEAPCHECK is required, for example: env HEAPCHECK=normal ./objs/srs -c %s",
               cpath.c_str());
    } else {
...
void show_macro_features() {
  ...
  if (true) {
    ...
    ss << ", conf:" << _srs_config->config() << ", limit:" << _srs_config->get_max_connections()
       << ", writev:" << sysconf(_SC_IOV_MAX)
       << ", encoding:" << (srs_is_little_endian() ? "little-endian" : "big-endian")
